...
export const CharacterDisplayCard: React.FC<CharacterDisplayCardProps> = (props) => {
    ...
    const [isHoodOff, setIsHoodOff] = useState(false);

    // Reset hood on avatar change
    useEffect(() => {
        setIsHoodOff(false);
    ...
    return (
        <div className={`w-full max-w-[440px] bg-white dark:bg-slate-900 border-4 ${rarityStyle.border} rounded-3xl relative shadow-2xl transition-all duration-500 group`}>
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {currentAvatarId === 'grand_wizard' && equippedArmorId === 'a_void_cloak' && (
                            <button onClick={() => setIsHoodOff(!isHoodOff)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-white text-[10px] uppercase font-bold px-2 py-1 rounded-full transition-colors flex items-center gap-1">
            ...
            {/* Main Image Area */}
            <div className="relative w-full h-[440px] z-10 -mt-4 bg-white dark:bg-slate-900 overflow-hidden">
                {/* BACKDROP */}
                {(() => {
                    ...
                    const style = (scale !== 100 || offsetX !== 0 || offsetY !== 0) ? { transform: `scale(${scale / 100}) translate(${offsetX}px, ${offsetY}px)`, transformOrigin: 'center center' } : undefined;
                ...
                {activeAccessoryItem && (() => {
                    ...
                    return (
                        <div className="absolute transition-all duration-500 hover:scale-110 group/pet w-max"
                            style={{
                                top: `${top}%`,
                                right: `${right}%`,
                        ...
                        >
                            <div className="relative animate-bounce-slow">
                                <img src={activeAccessoryItem.imageUrl} className="relative w-28 h-28 object-contain pixelated z-10" />
                            </div>
                        </div>
                    );
