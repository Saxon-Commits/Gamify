...
import { LayoutDashboard, Sword, Network, User, Settings, Coins, Zap, Sparkles, ShoppingBag, Package, Book, Diamond } from 'lucide-react';
...
const NavItem: React.FC<NavItemProps> = ({ to, icon: Icon, label, active, variant = 'default' }) => {
      ...
      <Link to={to} className="relative group select-none flex items-center justify-center w-[110px] h-[36px]">
        ...
        <div
          className={`
            ...
            ${active
              ...
              : 'bg-gradient-to-b from-slate-700 to-slate-800 opacity-50 group-hover:opacity-100 group-hover:from-amber-700 group-hover:to-amber-900 group-hover:shadow-[0_0_15px_rgba(245,158,11,0.3)]'
            }
          `}
          style={{
        ...
        <div
          className={`
            ...
            ${active ? 'bg-slate-900' : 'bg-slate-950'}
          `}
          style={{
            clipPath: 'polygon(0% 50%, 8px 30%, 20px 0%, calc(100% - 20px) 0%, calc(100% - 8px) 30%, 100% 50%, calc(100% - 8px) 70%, calc(100% - 20px) 100%, 20px 100%, 8px 70%)'
          }}
        >
          <Icon size={16} className={`transition-colors duration-300 ${active ? 'text-amber-400' : 'text-slate-500 group-hover:text-amber-200'}`} />
          <span className={`font-serif italic font-bold text-xs tracking-widest transition-colors duration-300 ${active ? 'text-amber-100' : 'text-slate-400 group-hover:text-amber-100'}`}>
            {label}
          </span>
        </div>
  ...
  return (
    <Link
      to={to}
      className={`relative flex items-center justify-center w-[110px] h-[36px] rounded-full transition-colors duration-500 ease-out ${active ? 'text-amber-100' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
    >
...
export const Layout: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 text-slate-100">
      ...
      <header className="h-16 bg-slate-900/50 backdrop-blur-xl border-b border-slate-800/50 px-6 flex items-center justify-between sticky top-0 z-50">
        ...
        <nav className="flex items-center gap-1.5 bg-slate-950/50 px-2 py-1.5 rounded-full border border-slate-800/50">
          {NavLinks.map(link => (
            <NavItem
              key={link.to}
              to={link.to}
        ...
        <div className="flex items-center space-x-4 hidden md:flex">
          <div className="flex flex-col items-end mr-4 gap-1.5">
            <div className="flex items-baseline gap-2">
              <span className="text-sm font-black text-white leading-none tracking-tight">Lvl {stats.level}</span>
              <span className="text-[10px] text-slate-500 font-mono">{stats.xp}/{stats.xpToNext} XP</span>
            </div>
            <div className="w-32 h-2 bg-slate-800/50 border border-slate-700/50 rounded-full overflow-hidden p-[1px]">
              <div
          ...
          <div className="flex items-center space-x-3 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5">
            <ResourceItem icon={Coins} value={stats.gold} label="Gold" description="Currency for items and gear." color="text-amber-400" />
          ...
          <button
            onClick={() => toggleNotes()}
            className={`p-2 rounded-lg transition-colors ${isNotesOpen ? 'bg-amber-500/20 text-amber-400' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
            title="Open Notes"
          >
            <StickyNote size={20} />
          </button>
