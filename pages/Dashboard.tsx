        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xl">
          <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Active Side Quests</h2>
            </div>
            <Link to="/app" className="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors uppercase flex items-center">
              View All <ArrowRight size={10} className="ml-1" />
            </Link>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {dailyTasks.length > 0 ? dailyTasks.map(task => (
              <div key={task.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                <div className="flex items-center space-x-3">
                  <button
                    onClick={() => completeTask(task.id)}
                    className="text-slate-400 dark:text-slate-600 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors"
                  >
                    <Circle size={18} />
                  </button>
                  <div>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{task.name}</p>
                    <div className="flex space-x-2 text-[10px] text-slate-500 uppercase font-bold">
                      <span className="text-indigo-500 font-bold">{task.xpReward} XP</span>
                      <span className="text-yellow-600 dark:text-yellow-500/80">{task.goldReward}G</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xl">
          <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Main Quest Progress</h2>
            </div>
          </div>
              ...
              return (
                <div key={project.id} className="space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">{project.name}</h3>
